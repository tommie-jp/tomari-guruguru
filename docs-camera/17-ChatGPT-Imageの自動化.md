...
```text
1) 基準キャラ画像を1枚確定（手動 or best-of-N で人が選ぶ）
2) その基準絵を reference にして images.edit で A シート生成
3) A シート（or 基準絵）を reference に B〜F を生成 ← 全6枚を同一キャラに固定
4) 6枚 → 既存 slice_character_sheets.py に流す（ここは完全自動）
```
