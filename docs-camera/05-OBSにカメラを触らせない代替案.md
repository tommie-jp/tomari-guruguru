...
```text
[iPhone] Safari / HTTPS         カメラ + MediaPipe で推論。設定UI(tweaks)もここ。
  └ producer                    「状態フレーム」を WSS 送信
        │
        ▼
   [中継サーバ] Node "ws"        信号(state)は計算せず素通し。接続管理と「設定要求・接続通知」の仲介だけ行う
        │
        ▼
[OBS CEF] camera.html?rx=ws     状態を受信 → 平滑化 → 画像/transform 適用（カメラ・推論・UIなし）
```
...
```text
byte0  flags : bit0 faceDetected / bit1-3 sheet(0..5 = A..F) / bit4- 予備
--- faceDetected=true のときだけ続く ---
byte1  int8   colX  (-1..1 → ×127)   グリッド列ターゲット
byte2  int8   rowY  (-1..1 → ×127)   グリッド行ターゲット
byte3  int8   tilt  (deg, 確定ゲイン後)
byte4  int8   slideX(vw, 確定後)
byte5  int8   slideY(vh, 確定後)
byte6  uint8  zoom  ((scale-0.6)/1.2 → ×255)
```
...
`camera-app.jsx` のループは複雑なので、ローカル版(camera.html) / tx / rx で式を3重に
...
ローカルの camera.html も同じ経路にすれば実装が一本化される。
...
RELAY_CERT=cert.pem RELAY_KEY=key.pem npm run relay      # wss://0.0.0.0:8787
#   ↑ 同一PC内の検証だけなら平文でも可: npm run relay     # ws://0.0.0.0:8787

# 2) dev サーバを HTTPS で（iPhone から開くため。同じ鍵を渡す）
VITE_TLS_CERT=cert.pem VITE_TLS_KEY=key.pem npm run dev
```

- iPhone（送信側）: `https://<PCのLAN名/IP>:5173/camera.html?tx`
  カメラ＋推論＋設定UI。画面下に「CEF 接続中（n）／未接続」を表示する。
- OBS の CEF（受信側）: ブラウザソースに `https://<同ホスト>:5173/camera.html?rx&obs=1`
  カメラを起動せず受信した動きで描画。`?obs=1` で透過オーバーレイ。
