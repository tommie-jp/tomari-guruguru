...
[iPhone] Safari / HTTPS         カメラ + MediaPipe で推論。設定UI(tweaks)もここ。
  └ producer                    「状態フレーム」を WSS 送信
   ...
   [中継サーバ] Node "ws"        信号(state)は計算せず素通し。接続管理と「設定要求・接続通知」の仲介だけ行う
        │
        ▼
[OBS CEF] index.html?rx=ws     状態を受信 → 平滑化 → 画像/transform 適用（カメラ・推論・UIなし）
```
...
`camera2-app.jsx` のループは複雑なので、ローカル版(index.html) / tx / rx で式を3重に
...
`src/relay-mode.js`、`src/camera2-app.jsx`（tx/rx 分岐）。`?tx`/`?rx`（`=ws` 任意）で役割を切り替える。
