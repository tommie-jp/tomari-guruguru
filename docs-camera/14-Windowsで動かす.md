...
開放も不要**（iPhone を使う別端末構成は [11-WS中継の接続手順.md](11-WS中継の接続手順.md) を参照）。
...
tx 側の画面下に「**OBS接続中（1）**」が出れば結線 OK。tx で顔を振る・口を開けると rx が同調する。
...
| WS 中継だけ動かす（standalone・別マシン用） | `npm run relay`（静的配信なし・`127.0.0.1:8787`） |

ポートを 8787 以外にしたら、rx/tx の URL もそのポートに合わせる。中継 URL は
「ページと同一オリジン（同じ `host:port`）＋専用パス `/__relay`」に自動解決されるので、
`?relay=` を足す必要はない（配信 / EXE はページも WS も同じ 8787 ポートなので従来どおり動く）。

> 開発（`npm run dev`）では Vite が WS 中継を同居させるため、別途 `npm run relay` を立てる
> 必要はない（`npm run dev` だけで tx/rx できる）。HMR と衝突しないよう中継は専用パス
> `/__relay` を使う。dev 同居の中継は既定で loopback 限定（Vite を `0.0.0.0` で公開しても
> 中継は loopback のみ）。LAN に公開したいときは `RELAY_EXPOSE=1` で明示的にオプトインする
> （無認証 WS なので信頼できる私設網のみ。流れるのは数値 pose だけだが、偽フレーム注入や
> 盗聴は可能）。
...
詳細は [10-OBSでライブ配信.md](10-OBSでライブ配信.md) を参照。

## つまずいたら

- **カメラが出ない**: tx を `http://localhost:...` で開いているか（`127.0.0.1` でも可）。
  IP 直打ちや `file://` は secure context 外でカメラ不可。
- **rx が真っ黒 / 動かない**: tx 側が「OBS接続中」になっているか。OBS のブラウザソースを
...
[53-単体EXEにする.md](53-単体EXEにする.md) を参照。
