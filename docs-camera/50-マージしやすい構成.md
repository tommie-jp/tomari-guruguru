...
関連: [51-フォーク元の取り込み.md](51-フォーク元の取り込み.md)
...
フォーク追加分のうち、**純粋な追加**である localStorage 永続化ヘルパー
（`tweaksStorageKey` / `loadTweaks` / `saveTweaks` — 初期実装時点。現在は多数のキーに拡張済み）を
`src/use-tweaks.js` に移し、import するだけにした。

- ヘルパーは本家に存在しない＝**衝突しようがない**ファイルへ退避。
- `tweaks-panel.jsx` に残るフォーク差分（`useTweaks` の永続化呼び出し・4要素返し（`[values, setTweak, resetTweaks, themes]`）、
...
| `.gitignore` | 数行 | 小さく、衝突しても自明 |
| `app.jsx` / `talk-app.jsx` | 現在は差分なし | フォーク固有の変更はなく、番兵マーカーも存在しない |
