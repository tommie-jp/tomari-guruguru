...
新規追加ファイル（`index.html`(旧 camera2) / `tracking.html` / `src/face/*` / `src/tracking/*` /
...
input: {
  ...
  index_old: resolve(import.meta.dirname, 'index_old.html'),// 互換リダイレクト（消さない）
  camera2:   resolve(import.meta.dirname, 'camera2.html'),   // 互換リダイレクト（消さない）
},
```
