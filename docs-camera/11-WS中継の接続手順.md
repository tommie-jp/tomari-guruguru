...
> `npm run relay` を別ターミナルで立てる必要はない。**既定は localhost モード**（平文・localhost）
> なのでフラグは不要。明示するなら `--localhost`（`-l` / `--no-tls` / `NO_TLS=1` でも可）。
> スクリプトは起動前に詰まったポートを自動解放する。別端末から繋ぐ手順B は `--tailscale`。
...
```bash
./doStartDev.sh --tailscale   # dev（HTTPS）＋ WS 中継を同居して起動
```

`--tailscale`（`-t` / `--tls` / `TLS=1` でも可）でモードB に切り替わり、FQDN 取得・証明書解決・
HTTPS 配信まで自動で行う。tailscale が無い／証明書を出せないときは警告のうえ平文（http＝モードA）に
フォールバックする。

> **別端末（iPhone/OBS）から繋ぐとき**: 同居中継は既定 loopback 限定だが、`--tailscale`（モードB）は
> 別端末配信の明示なので `doStartDev.sh` が中継を tailnet へ公開する（`RELAY_EXPOSE=1` を自動設定）。
> 無認証 WS なので tailscale など ACL で閉じた私設網でのみ使うこと。素の `npm run dev` を手で使うなら
> `RELAY_EXPOSE=1 npm run dev` を付ける。
>
