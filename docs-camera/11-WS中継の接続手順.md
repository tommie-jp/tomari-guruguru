...
`./doStartDev.sh` は既定（`--both`）で vite dev と `dev-tls-proxy.mjs` を並行起動する（`npm run dev` 単体とは別物）。Vite が WS 中継を同居させる（専用パス `/__relay`）ので、
...
./doStartDev.sh  # http://localhost:5173（既定=both: vite dev + dev-tls-proxy.mjs）
```

> `./doStartDev.sh` が WS 中継を専用パス `/__relay` で同居させるので、`./doServer.sh` /
> `npm run relay` を別ターミナルで立てる必要はない。**既定は both モード**で、PC は
> `http://localhost:5173`、iPhone/OBS は `https://<FQDN>:5174` を1回の起動で同時に使える
...
- tx タブの画面下に「**OBS接続中（1）**」が出る（rx を閉じると「OBS未接続」）。
...
```bash
./doStartDev.sh               # 既定=both: PC=http://localhost:5173 と iPhone/OBS=https://<FQDN>:5174 を同時
```

**既定の both モード**は、vite は http のまま起動する（WSL では 0.0.0.0 に自動バインドされ Windows Chrome から localhost で届く）。TLS リバースプロキシ
（`server/dev-tls-proxy.mjs`）を tailscale IP に立てて HTTPS を終端し、外部向けポートは 5174 になる。FQDN/IP/証明書は
...
- iPhone(tx): `https://wsl40.taild830ae.ts.net:5174/index.html?tx`
- OBS の CEF(rx): ブラウザソースに `https://wsl40.taild830ae.ts.net:5174/index.html?rx`
  （`?rx` は OBS 用に既定で透過＋UI 非表示。`&obs=1` を付けても同じ）

iPhone 側に「**OBS接続中（n）**」が出れば結線 OK。設定（感度・口・ズーム等）は iPhone で変更すると、
...
- relay URL は自動でページと同一オリジンの `wss://wsl40.taild830ae.ts.net:5174/__relay` に解決される
  （dev が中継を同居）。証明書はページ（:5174）のものを使うので FQDN と一致する。別マシン中継を使う
  ときだけ `&relay=wss://<host>:8787` を明示する。
- 動作確認: `curl https://wsl40.taild830ae.ts.net:5174/index.html` が **`-k` なしで 200**
...
- **別端末（iPhone/OBS）から繋がらない**: `./doStartDev.sh`（既定の both）を使っていれば
  `RELAY_EXPOSE=1` は不要（TLS proxy が loopback 経由で中継へ繋ぐ）。
  LAN に直接公開する場合（`--tailscale` / 旧モード）は中継が loopback 限定になるため
  `RELAY_EXPOSE=1` を明示するか `./doStartDev.sh --tailscale` を使う（自動で付く）。
  素の `npm run dev` のみで別端末を繋ぐなら `RELAY_EXPOSE=1 npm run dev`。
  なお既定の中継先はページと同一オリジンの `/__relay`。別ポート/別マシンの
  standalone 中継（`./doServer.sh`）を使うときは各端末に `&relay=ws(s)://<host>:8787` を明示する。
- **rx が動かない**: tx 側で「OBS接続中」になっているか、ブラウザのコンソールに WS エラーが
