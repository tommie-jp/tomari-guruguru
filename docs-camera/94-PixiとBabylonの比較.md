...
カメラ版アバターの描画を現状の **PixiJS v8** から **Babylon.js** へ替えるか検討した比較メモ（決定済み → Babylon 採用、95 参照）。
...
| 本アバター適性（当時の評価） | ○ 最適・軽量 | △ オーバースペック | **Pixi** |
...
- **3D を使わない現状は Pixi v8 維持が合理的**（当時の評価）。演出パリティはほぼ互角なのに、Babylon 化で得る新規演出は現状ほぼ無く、サイズ・起動・メモリ・CPU は一貫して Pixi が軽い（OBS / CEF は 1 ソース = 1 プロセスで毎回新規ロード＝軽さがそのまま効く）。
...
> [追記] 2.5D 視差・tommieChat 統合を理由に Babylon 移行を決定（95）。以下は移行決定前の評価記録。
