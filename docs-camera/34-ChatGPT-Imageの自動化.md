...
関連: [32-アバター画像の生成AI.md](32-アバター画像の生成AI.md)（無料版の代替先）/
[31-アバターの追加.md](31-アバターの追加.md) / [templates/](templates/)
...
- 本番は `gpt-image-2`（gen スクリプトの既定）。`gpt-image-1` 系は**無料では1回も叩けない**
  （org 本人確認＋有料 Tier 必須）ので、検証は無料代替で行う。

## 選択肢の比較

| 観点 | ① 公式 API（`gpt-image-2`）**推奨** | ② ブラウザ自動化（Playwright で Web 版操作） |
| --- | --- | --- |
...
./doGenPollinations.sh "..." --clean      # 検証後に出力を削除
```
...
- 4倍アップスケールは API 単体では不可（size 上限 1536）→ Real-ESRGAN 等で別途。
  Real-ESRGAN-GUI の **WebP 出力はそのまま `doAvatarConvert.sh` に入力できる**（png/webp 両対応）。
  例: `realesrgan-x4plus-anime` で4倍 → WebP 保存 → `./doAvatarConvert.sh assets/<id> <id> 2000 100`。
...
| モデル / 品質 | 1枚 | 6枚＝1キャラ |
| --- | --- | --- |
| `gpt-image-2` | （2026-06 時点 要確認） | （2026-06 時点 要確認） |
...
def build_character(prompt_of):                 # prompt_of(state) -> str
    ...
    subprocess.run(["python3", "tools/slice_character_sheets.py",
