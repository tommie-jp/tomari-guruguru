...
camera2.html に新しいアバターを追加する手順メモ。**登録は JSON ではなく
`src/character-config.js`（JS のレジストリ `AVATAR_DEFS`）を編集する。**
`public/default-themes/*.json` は Tweaks のテーマで、アバターとは無関係。

関連: [11-camera2のエフェクト.md](11-camera2のエフェクト.md) / [04-OBSでライブ配信.md](04-OBSでライブ配信.md)

対象は camera2 のみ。app / talk / camera は単一アバターのまま。
...
これで `avatars` レジストリに自動で載り、camera2 のセレクタ・`?avatar=<id>`（OBS シーン固定用）・
...
npm run dev           # camera2.html でセレクタ表示と切替を確認
npm run verify:pages  # build 後、アバター別シートの存在を検査
```

## 編集「しない」もの

- `public/default-themes/camera.html.json` … camera.html の Tweaks テーマ（アバター無関係）。
