...
- **受信側 rx** … OBS のブラウザソース。カメラは使わず、tx の動きで描画するだけ。
...
2. 既定ブラウザで送信側 tx が開く → `http://localhost:8787/?tx`。
...
[04-OBSでライブ配信.md](04-OBSでライブ配信.md) を参照。

## 接続を確認する

- tx（ブラウザ）の画面下に「**CEF 接続中（1）**」が出れば結線 OK。
...
- **OBS が真っ黒 / 動かない**: tx 側が「CEF 接続中」になっているか確認。OBS のブラウザソースを
...
- ソースからビルドして動かす（開発者向け）: [09-Windowsで動かす.md](09-Windowsで動かす.md)
- 単体 EXE の作り方: [10-単体EXEにする.md](10-単体EXEにする.md)
- OBS にカメラ自体を触らせる構成: [04-OBSでライブ配信.md](04-OBSでライブ配信.md)
- 公開済み zip の自動テスト: [20-リリースの動作テスト.md](20-リリースの動作テスト.md)
