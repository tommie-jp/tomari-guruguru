...
ステータス: **WS 中継方式（下記「iPhone 起点の構成」）は実装済み**（`?tx`/`?rx` ＋ dev 同居中継、
ユニット/結線テスト済み・Win11 実機での tx/rx 通し確認済み）。従来の `--enable-media-stream` 方式も引き続き利用可。
...
- 送信側（Chrome）: 既存のトラッキングをそのまま使い、コンパクトな状態フレーム
  （colX/rowY/tilt/slideX/slideY/zoom/sheet 等）を WS で送信する。
- 受信側（OBS ページ）: 新モード（例 `index.html?rx`）で WS を購読し、受け取った
  状態フレーム（colX/rowY/tilt 等）を **`applyState` で反映**（カメラ・推論を起動しない）。
...
- WS 中継（`vite-plugin-relay.mjs`）: dev サーバに同居。状態フレームを全受信クライアントへブロードキャスト。standalone 用は `server/relay.mjs` / `npm run relay`。
- 送信モード（`src/relay-mode.js`, `src/face/relay-client.js`）: `?tx` でトラッキング結果を状態フレームとして WS 送信。
- 受信モード分岐（`src/relay-mode.js`, `src/camera-app.jsx`）: `?rx` でカメラを起動せず、受信した状態フレームを `applyState` で反映。
...
- **接続通知**: CEF の接続/切断をサーバが iPhone へ通知 → iPhone 画面に「OBS接続中／未接続」を表示。
...
2. リレーサーバ（`ws`）。`state` の broadcast に加え、接続管理＋制御の仲介を持つ:
   ...
   CEF の接続/切断を iPhone へ通知。`?tx`/`?rx` で役割を切り替え。Vite dev に同居可。
...
```bash
# 推奨: dev は既定の both でOK（localhost http と tailscale https を同時。接続手順は docs-camera/11-WS中継の接続手順.md）
./doStartDev.sh              # dev : localhost(http):5173 と https://<FQDN>:5174 を同時（TLS proxy 同居）
./doServer.sh --tailscale    # 中継を別マシンに分離するときだけ standalone（WSS・0.0.0.0）
#   ↑ 同一PC内の検証だけで localhost に固定したいなら: ./doStartDev.sh -l
...
- iPhone（送信側）: `https://<PCのLAN名/IP>:5174/index.html?tx`
  カメラ＋推論＋設定UI。画面下に「OBS接続中（n）／未接続」を表示する。
- OBS の CEF（受信側）: ブラウザソースに `https://<同ホスト>:5174/index.html?rx`
