...
```bash
# ターミナル1: 中継（WSS）。別端末から繋ぐので --host 0.0.0.0 で全インターフェイスにバインド。
RELAY_CERT=wsl40.taild830ae.ts.net.crt RELAY_KEY=wsl40.taild830ae.ts.net.key \
  npm run relay -- --host 0.0.0.0
# ターミナル2: dev（HTTPS）
VITE_TLS_CERT=wsl40.taild830ae.ts.net.crt VITE_TLS_KEY=wsl40.taild830ae.ts.net.key npm run dev
```

> 中継サーバの既定バインドは `127.0.0.1`（同一PC・loopback）。iPhone など**別端末から繋ぐ
> 手順 B では `--host 0.0.0.0` が必須**（付け忘れると iPhone から届かない）。`npm run relay --
> --host 0.0.0.0` の `--` 以降がサーバへの引数。環境変数 `RELAY_HOST=0.0.0.0` でも同じ。
