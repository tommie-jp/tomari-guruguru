...
`index.html?tx`（送信）と `index.html?rx`（受信）を中継サーバ（`npm run relay`）でつなぐ手順。
...
中継まわりの抜粋。`?obs` / `?shadow` / `?avatar` / `?camera` を含む**全パラメータ**は
[16-URLパラメータ一覧.md](16-URLパラメータ一覧.md) にまとめてある。
