...
#
# 位置づけ: 通常の dev（npm run dev）は vite が WS 中継を同居させる（vite-plugin-relay.mjs / 専用
# パス /__relay）ので、これ単体を立てる必要は無い。OBS を別PCに置く・中継を別ポートにする等、別マシン/
...
通常の dev（npm run dev）は vite が WS 中継を同居（/__relay）させるので、これ単体は不要。
OBS を別PCに置く・別ポートで中継する等のときに使う standalone サーバ。中継ロジックは
server/relay-core.mjs を vite-plugin-relay と共有する。

中継サーバは tx(?tx) の状態フレーム/config を rx(?rx, OBS の CEF) へ素通しするだけ。
別マシン中継時は rx 側で ?relay=ws(s)://<host>:<port> を明示する（dev とモードを揃える）。
dev 同居中継の LAN 公開は RELAY_EXPOSE=1（既定 loopback。無認証 WS なので私設網のみ）。
