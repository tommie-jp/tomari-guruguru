...
describe('computeTiltYawComp（左右を向いたときのかしげ混入補正の逆算）', () => {
  ...
  it('横向きで roll が大きくても飽和しない（±4 まで許容）', () => {
    // プロファイル想定: roll=0.95, yaw=0.7 → 0.95/0.7 ≈ 1.36（±1 だと 1 で飽和し傾きが残る）
    expect(computeTiltYawComp({ roll: 0.95, yaw: 0.7 })).toBeCloseTo(1.36, 2);
  });

  it('±4 でクランプ・yaw 振り不足は null・非有限は null', () => {
    expect(computeTiltYawComp({ roll: 3, yaw: 0.1 })).toBe(4); // 30 → 4
